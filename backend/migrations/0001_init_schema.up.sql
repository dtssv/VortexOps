...
-- pgvector 用于 AI 助手知识库向量检索（RAG）。
-- 若服务器未安装 pgvector 二进制（如非 pgvector/pgvector 镜像），
-- CREATE EXTENSION 会失败；放于 DO 块中捕获，避免整个迁移被中断。
-- 此时 vo_kb_chunks.embedding 仍可创建（见下方 DO 块），仅向量检索功能不可用。
DO $$
BEGIN
    CREATE EXTENSION IF NOT EXISTS "vector";
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'pgvector 扩展不可用，跳过向量检索能力。错误：%', SQLERRM;
END $$;
...
-- 顶级保留：dashboard（总览）不归组。
-- 注：AI 诊断已迁移为场景化按钮 + 全局浮窗，不再作为独立菜单。
...
-- 15.2b 顶级叶子菜单
-- 注：AI 诊断已迁移为「构建失败/启动失败」场景下的按钮入口 + 全局浮窗 AI 助手，
-- 不再作为独立菜单入口。menu:diagnosis:view 权限保留，用于 gating 上下文诊断接口。
...
FROM (VALUES
  ...
  ('grp-admin',    'knowledge-base',  'AI 知识库', '/admin/knowledge-base','book',         'menu','platform',750,true,'kb:manage'),
  -- 个人
...
-- ============================================================================
-- 16. AI 助手（知识库 RAG / 用户画像 / 对话会话）
-- ============================================================================
-- 说明：
--   - 知识库采用 pgvector 向量检索（RAG），支持文档分块、向量化、相似度召回。
--   - 用户画像在多轮对话中持续更新，用于个性化回答。
--   - 对话会话持久化，支持上下文摘要与实体记忆，跨会话保留。

-- ---------- 16.1 知识库分类 ----------
CREATE TABLE vo_kb_categories (
    id          BIGSERIAL PRIMARY KEY,
    uuid        UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    name        VARCHAR(128) NOT NULL,
    code        VARCHAR(64) NOT NULL UNIQUE,
...
-- ---------- 16.2 知识库文档 ----------
CREATE TABLE vo_kb_documents (
    id             BIGSERIAL PRIMARY KEY,
    uuid           UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    category_id    BIGINT REFERENCES vo_kb_categories(id),
    title          VARCHAR(255) NOT NULL,
    source_type    VARCHAR(32) NOT NULL DEFAULT 'manual', -- manual / url / markdown / faq
    source_url     VARCHAR(1024),
    content        TEXT NOT NULL,                          -- 原始全文
    tags           JSONB NOT NULL DEFAULT '[]'::jsonb,     -- 字符串数组
    chunk_count    INT NOT NULL DEFAULT 0,                 -- 分块数量
    status         VARCHAR(16) NOT NULL DEFAULT 'active',  -- active / indexing / archived
    version        INT NOT NULL DEFAULT 1,
    created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
    created_by     BIGINT,
    updated_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX idx_kb_documents_category ON vo_kb_documents (category_id) WHERE deleted = false;
CREATE INDEX idx_kb_documents_status ON vo_kb_documents (status) WHERE deleted = false;

-- ---------- 16.3 知识库文档分块（向量索引） ----------
-- embedding 维度：OpenAI text-embedding-3-small=1536；其它兼容服务可能不同。
-- 向量维度通过应用层保证一致；若使用其它模型需重建索引。
-- 说明：embedding 列与 ivfflat 索引依赖 pgvector 扩展；若扩展不可用则降级为
--       仅存储文本（embedding 列改用 TEXT 占位），应用层 SearchByVector 会失败并返回友好错误。
CREATE TABLE vo_kb_chunks (
    id           BIGSERIAL PRIMARY KEY,
    uuid         UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    document_id  BIGINT NOT NULL REFERENCES vo_kb_documents(id) ON DELETE CASCADE,
    chunk_index  INT NOT NULL,
    content      TEXT NOT NULL,
    embedding    BYTEA,                       -- 默认占位；若 pgvector 可用则下方 DO 块改为 vector(1536)
...
CREATE INDEX idx_kb_chunks_document ON vo_kb_chunks (document_id) WHERE deleted = false;
...
-- ---------- 16.4 用户画像 ----------
-- 在多轮对话中持续更新，用于个性化回答（如「运维知识丰富」「资深 Java 工程师」）。
CREATE TABLE vo_user_profiles (
    id                  BIGSERIAL PRIMARY KEY,
    uuid                UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    user_id             BIGINT NOT NULL UNIQUE,            -- 一个用户一份画像
    expertise_level     VARCHAR(32) NOT NULL DEFAULT 'unknown', -- beginner / intermediate / advanced / expert
    roles               JSONB NOT NULL DEFAULT '[]'::jsonb,     -- 字符串数组，如 ["java_engineer","sre","devops"]
    domains             JSONB NOT NULL DEFAULT '[]'::jsonb,     -- 擅长领域，如 ["kubernetes","spring","redis"]
    communication_style VARCHAR(32) NOT NULL DEFAULT 'balanced', -- concise / detailed / balanced
    preferred_language  VARCHAR(16) NOT NULL DEFAULT 'zh-CN',
    summary             TEXT,                               -- LLM 生成的人物画像摘要
    interaction_count   INT NOT NULL DEFAULT 0,            -- 累计对话轮次
...
CREATE INDEX idx_user_profiles_user ON vo_user_profiles (user_id) WHERE deleted = false;

-- ---------- 16.5 对话会话 ----------
CREATE TABLE vo_chat_sessions (
    id              BIGSERIAL PRIMARY KEY,
    uuid            UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    user_id         BIGINT NOT NULL,
    title           VARCHAR(255),                          -- 自动从首条消息生成
    scene           VARCHAR(32),                           -- 上下文场景（构建/发布/K8s 等，自动识别）
    summary         TEXT,                                  -- 长会话的上下文摘要（LLM 生成）
    entities        JSONB NOT NULL DEFAULT '{}'::jsonb,    -- 实体记忆：{build_id:123, app_id:456, ...}
...
CREATE INDEX idx_chat_sessions_user ON vo_chat_sessions (user_id, last_active_at DESC) WHERE deleted = false;

-- ---------- 16.6 对话消息 ----------
CREATE TABLE vo_chat_messages (
    id              BIGSERIAL PRIMARY KEY,
    uuid            UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    ...
    content         TEXT NOT NULL,
    intent          JSONB,                                 -- 意图识别结果（assistant 消息）
    tools           JSONB,                                 -- 工具调用记录（assistant 消息）
    "references"    JSONB,                                 -- RAG 引用（assistant 消息）
    latency_ms      INT,
    version         INT NOT NULL DEFAULT 1,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    created_by      BIGINT,
...
CREATE INDEX idx_chat_messages_session ON vo_chat_messages (session_id, id) WHERE deleted = false;
...
-- ---------- 16.8 知识库默认分类（created_by=1：平台管理员） ----------
...
-- ---------- 16.9 默认知识库内容（FAQ 入门） ----------
-- 提供基础 FAQ 文档，便于开箱即用；管理员可后续编辑或扩展。
INSERT INTO vo_kb_documents (category_id, title, source_type, content, tags, chunk_count, status, created_by, updated_by) VALUES
 ((SELECT id FROM vo_kb_categories WHERE code='build'), '构建失败常见原因', 'manual',
  '## 构建失败常见原因\n\n### 镜像拉取失败\n- 检查基础镜像仓库地址是否正确\n- 检查凭证是否有效（Harbor/Registry 凭证）\n- 检查网络连通性\n\n### Dockerfile 语法错误\n- 检查指令拼写\n- 检查 COPY/ADD 路径是否存在\n- 检查多阶段构建引用的镜像是否存在\n\n### 依赖拉取超时\n- Maven/Gradle/npm 仓库镜像配置\n- 网络代理配置\n- 依赖版本是否存在',
  '["build","dockerfile","failure"]', 0, 'active', 1, 1),
 ((SELECT id FROM vo_kb_categories WHERE code='k8s'), 'Pod 启动失败排查指南', 'manual',
 ...
 ((SELECT id FROM vo_kb_categories WHERE code='release'), '发布失败排查指南', 'manual',
    ...
    FOR t IN
        ...
        WHERE column_name = 'updated_at'
          ...
          AND table_name IN (
            ...
            'vo_kb_categories','vo_kb_documents','vo_kb_chunks',
            'vo_user_profiles','vo_chat_sessions','vo_chat_messages'
          )
    LOOP
        EXECUTE format(
